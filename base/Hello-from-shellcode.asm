...
search_WriteConsoleA_loop:
    ...
    sub edi, 64
    mov dword [edi], 'Writ'
    mov dword [edi+4], 'eCon'
    mov dword [edi+8], 'sole'
	mov byte [edi+12], 'A'
    mov byte [edi+13], 0
    compare_strings eax, edi, eax
	SUPER_POP edx, ecx, edi, esi
    ...
    add esi, 4 ; <- переход к адресу след. имени
    inc edx ; <- инкрементирование индекса
    loop search_WriteConsoleA_loop ; выполняется ecx раз
...
payload:
    ...
    mov dword [edi], 'Hell'
    mov dword [edi+4], 'o, W'
    mov dword [edi+8], 'orld'
    mov byte [edi+12], '!'
    mov byte [edi+13], 0x0A  ; \n
    mov byte [edi+14], 0     ; Нулевой байт (конец строки)
